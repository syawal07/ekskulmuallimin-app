...
export default async function AdminSiswaPage({
  ...
  // 1. AMBIL DAFTAR EKSKUL (FIX: Pakai 'distinct' biar gak double)
  const allExculs = await prisma.excul.findMany({
    orderBy: { name: 'asc' },
    select: { id: true, name: true },
    distinct: ['name'] // <--- INI SOLUSI AGAR DROPDOWN TIDAK KEMBAR
  })

  // 2. LOGIKA FILTER (PENGGANTI TAB)
  const whereClause: Prisma.StudentWhereInput = {
    isActive: true,
    name: { contains: search, mode: "insensitive" }
  }

  // Jika Filter Dropdown dipilih, query hanya mengambil siswa ekskul tersebut
  if (filterExculId) {
