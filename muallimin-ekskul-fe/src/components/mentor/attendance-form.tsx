...
export default function AttendanceForm({
  ...
  const defaultDate = initialDate
    ? typeof initialDate === "string"
      ...
      : new Date(initialDate).toISOString().substring(0, 10)
    : new Date().toISOString().substring(0, 10);

  const draftKey = `draft_presensi_${exculId}_${defaultDate}`;
  ...
  const getCurrentNotes = (studentId: string) => {
    if (localData[studentId]?.notes !== undefined)
      return localData[studentId].notes;
          ...
          {students.map((s) => {
            ...
            return (
              <div key={`hidden-${s.id}`}>
                <input
                  name={`status-${s.id}`}
                  value={getCurrentStatus(s.id)}
                  readOnly
                />
                <input
                  name={`notes-${s.id}`}
                  value={getCurrentNotes(s.id)}
                  readOnly
                />
              </div>
            );
            ...
            <div className="space-y-4">
              {paginatedStudents.length === 0 ? (
                ...
                paginatedStudents.map((student, index) => (
                  <div
                    key={student.id}
                    className="p-4 rounded-xl border border-slate-200 bg-white hover:border-primary/30 hover:shadow-md transition-all duration-200 flex flex-col gap-4"
                  >
                    <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
                      ...
                      <div className="flex-1">
                        <p className="font-bold text-slate-900 text-base leading-tight line-clamp-1">
                          {student.name}
                        </p>
                      ...
                      <div className="flex-1">
                        ...
                        >
                          <div className="relative">
                            ...
                            />
                            <Label
                              htmlFor={`h-${student.id}`}
                              className="flex flex-col items-center justify-center gap-1 p-2 rounded-lg border-2 border-slate-100 cursor-pointer hover:bg-green-50 peer-data-[state=checked]:bg-green-50 peer-data-[state=checked]:border-green-500 peer-data-[state=checked]:text-green-700 transition-all font-semibold text-xs sm:text-sm text-slate-500"
                            >
                              Hadir
                            </Label>
                          </div>
                          <div className="relative">
                            ...
                            />
                            <Label
                              htmlFor={`i-${student.id}`}
                              className="flex flex-col items-center justify-center gap-1 p-2 rounded-lg border-2 border-slate-100 cursor-pointer hover:bg-yellow-50 peer-data-[state=checked]:bg-yellow-50 peer-data-[state=checked]:border-yellow-500 peer-data-[state=checked]:text-yellow-700 transition-all font-semibold text-xs sm:text-sm text-slate-500"
                            >
                              Izin
                            </Label>
                          </div>
                          <div className="relative">
                            ...
                            <Label
                              ...
                              className="flex flex-col items-center justify-center gap-1 p-2 rounded-lg border-2 border-slate-100 cursor-pointer hover:bg-blue-50 peer-data-[state=checked]:bg-blue-50 peer-data-[state=checked]:border-blue-500 peer-data-[state=checked]:text-blue-700 transition-all font-semibold text-xs sm:text-sm text-slate-500"
                            >
                              Sakit
                            </Label>
                          </div>
                          <div className="relative">
                            ...
                            <Label
                              ...
                              className="flex flex-col items-center justify-center gap-1 p-2 rounded-lg border-2 border-slate-100 cursor-pointer hover:bg-red-50 peer-data-[state=checked]:bg-red-50 peer-data-[state=checked]:border-red-500 peer-data-[state=checked]:text-red-700 transition-all font-semibold text-xs sm:text-sm text-slate-500"
                            >
                              Alpha
                            </Label>
                          </div>
            ...
            <AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Anda akan menghapus data presensi untuk tanggal{" "}
                    <b>{initialDate}</b>.<br />
                    Tindakan ini tidak bisa dibatalkan dan data akan hilang
                    permanen.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                {initialDate ? "Simpan Perubahan" : "Simpan Presensi"}
              </>
            )}
          </Button>
        </div>
            ...
            <form onSubmit={handleAddStudent} className="p-6 space-y-6">
              ...
              <div className="space-y-3 flex flex-col">
                <Label className="text-slate-700 font-semibold">
                  Cari Data Santri <span className="text-red-500">*</span>
                </Label>
                <Popover open={openCombobox} onOpenChange={setOpenCombobox}>
                  ...
                  >
                    <Command>
                      ...
                      <CommandList>
                        <CommandEmpty>
                          Nama santri tidak ditemukan di Data Induk.
                        </CommandEmpty>
                        <CommandGroup>
                          {allStudents?.map((s) => (
                            <CommandItem
                ...
                <p className="text-sm text-slate-500 mt-2 leading-relaxed">
                  Pilih nama dari Data Induk Madrasah. Jika nama santri
                  benar-benar tidak ada di pencarian, silakan hubungi Admin
                  Sekolah.
                </p>
              </div>
